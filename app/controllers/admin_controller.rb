class AdminController < ApplicationController
  ...
  def index
    @courses = current_account ? current_account.courses.active.order('DATE(starts_at) ASC').where(:status => "live") : Course.active.order('DATE(starts_at) ASC').where(:status => "live")
  ...
  def show
    ...
    @courses_last_month         = Course.where("extract( month from DATE(starts_at)) = 12").where("extract( year from DATE(starts_at)) = 2014").where('price != 0').where(:status => "live")
