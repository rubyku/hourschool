...
class HomeController < ApplicationController
  ...
  def search
    ...
    @courses = (@all_courses_in_city & Course.find(:all, :conditions => ["description LIKE ? AND date BETWEEN ? AND ?", search_string,date, date.advance(:weeks => 1)])).paginate(:page => params[:page], :per_page => 10)
    @top_suggestions =  Csuggestion.tally(
      {  :at_least => 1,
          :at_most => 10000,
          :limit => 10,
