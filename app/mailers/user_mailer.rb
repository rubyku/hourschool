class UserMailer < ActionMailer::Base
  ...
  def contact_all_students(current_user,course,message)
    @course = course
    @message = message
    mail(:to => course.students.map(&:email), :reply_to => course.teacher.email, :bcc => "ruby@hourschool.com, alex@hourschool.com", :subject => "Your teacher sent you a message!")
  end
  ...
  def contact_teacher(current_user, course, message)
    ...
    @course = course
    @message = message
    mail(:to => course.teacher.email, :reply_to => current_user.email, :bcc => "ruby@hourschool.com, alex@hourschool.com", :subject => "A student sent you a message!")
  end
  ...
  def send_registration_mail(user_email,user_name)
    @email = user_email
    @name = user_name
    mail(:to => user_email, :subject => "Welcome to the Business Communications Club!")
  end
   ...
   def send_course_reskilling_mail(user_email, user_name, course)
     ...
     @name = user_name
     @course = course
     mail(:to => user_email, :bcc => "ruby@hourschool.com, alex@hourschool.com", :subject => "Thanks for your interest in #{@course.title}!")
   end
